...
def train_models(data):
    ...
    # Regression models for Financial_Risk_Score
    reg_models = {
    ...
    # Classification models for Risk_Category
    cls_models = {
...
st.write(
    "Ứng dụng ML dự báo **Financial Risk Score** và **Risk Category** "
...
with tab1:
    ...
    if st.button("Dự báo rủi ro tài chính"):
        score_pred = trained["best_reg_model"].predict(input_df)[0]
        category_pred = trained["best_cls_model"].predict(input_df)[0]

        st.success("Dự báo hoàn tất")

        m1, m2 = st.columns(2)
        m1.metric("Financial Risk Score", f"{score_pred:.4f}")
        m2.metric("Risk Category", str(category_pred))
...
with tab2:
    ...
    if uploaded is not None:
        ...
        if missing_cols:
            ...
            new_df["Predicted_Financial_Risk_Score"] = trained["best_reg_model"].predict(X_new)
            new_df["Predicted_Risk_Category"] = trained["best_cls_model"].predict(X_new)
...
with tab3:
    ...
    c1.metric(
        "Best model - Financial Risk Score",
        trained["best_reg_name"]
    )

    c2.metric(
        "Best model - Risk Category",
        trained["best_cls_name"]
    )
    ...
    st.bar_chart(
        trained["reg_results"].set_index("Model")["R2"]
    )

    st.markdown("### Bảng so sánh ML cho Risk_Category")
    st.dataframe(trained["cls_results"], use_container_width=True)

    st.bar_chart(
        trained["cls_results"].set_index("Model")["F1_macro"]
    )
